...
public class WaveSwipeRefreshLayout extends ViewGroup
  ...
  private boolean onMoveTouchEvent(MotionEvent event, int pointerIndex) {
    if (mIsBeingDropped) {
      return false;
    }
   ...
   *
   * @see jp.co.recruit_lifestyle.android.widget.WaveView#setWaveColor(int)
   */
  ...
  private class ProgressAnimationImageView extends AnimationImageView {
    ...
    public ProgressAnimationImageView(Context context) {
      ...
      if (DisplayUtil.Companion.isOver600dp(getContext())) { // Make the progress be big
